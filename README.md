...
- **开通异步化（v2.8）**：`CreateAccount` 入队即返回（结账不卡），`AfterCronJob` 后台逐单跑编排；客户区显示「开通中…」进度。
- **IPAM 池组（v2.8）**：后台只加原始母段 + 设交付掩码范围，**按需对齐分配、不预切、释放不残碎**（空闲实时 = 母段 − 在用分配）。
- **线路实体（v2.8）**：线路决定交付哪段 IP（线路→池组）+ 落地交换机；与选机解耦。
- **VPN 池由插件下发 ROS（v2.8）**：`/ip pool` + L2TP server + PSK 一键幂等下发。
- **设备/资源角色化（v2.8）**：按 driver 角色（交换机/VPN 网关）显字段、分类资源、过滤选择。
...
modules/
  servers/owp_provision/
    owp_provision.php       # server 模块:开通/暂停/恢复/销户 + 客户区 + Admin 按钮
  ...
  addons/owp_provision/
    owp_provision.php       # addon 模块:设备 / 资源 / 占用 管理页
    hooks.php                # 桥接:require server 模块的 hooks.php
install/schema.sql           # 等效建表 SQL(参考;正常由 activate 自动建)
docs/INSTALL.md              # 完整安装与上线手册
```

两模块同名 `owp_provision`,共用 `mod_owp_provision_*` 表;addon 通过 `require_once` 引用 server 模块的 `lib/`(单一副本)。
...
捆绑的第三方库 **phpseclib**(MIT)与 **paragonie/constant_time_encoding**(MIT)各自版权归原作者,许可证见 `modules/servers/owp_provision/lib/sshv3/*-LICENSE.txt`。
