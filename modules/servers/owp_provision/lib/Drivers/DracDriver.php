...
class DracDriver
{
    private string $base;       // https://<host>:<port>
    private string $adminUser;
    private string $adminPass;
    private bool $dryRun;
    private int $timeout = 20;

    public function __construct(string $baseUrl, string $adminUser, string $adminPass, bool $dryRun = false)
    {
    ...
    public function createUser(string $username, string $password, string $role = 'Operator'): array
    {
        ...
        if ($slot <= 0) {
            return ['ok' => false, 'slot' => 0, 'error' => 'iDRAC 无空闲账号槽位'];
        }
